...
def printer(): 
    print() # Print the articles and their urls
    try:
...
while True:
    ...
    if action.upper() == "PREVIOUS" or action.upper() == "PREV":
        try:
            page_num -= 1
            if page_num < 0: page_num = math.ceil(len(response)/articles_per_page)
            printer()
        except: print("---No topic was selected!---")
